...
def render_ticket_card(ticket: Ticket):
    ...
    try:
        ...
        ticket_description = str(ticket.description or "").strip()
        
        # Extraer persona del título
        title_parts = ticket_title.split(" - ")
        display_title = (title_parts[0] or "Sin título").strip()
        person = (title_parts[1] if len(title_parts) > 1 else "").strip()
        ...
        ticket_description = ticket_description.replace("<", "&lt;").replace(">", "&gt;")
    ...
    """
        ...
        # Si algo falla, mostrar un mensaje de error transparente
        st.warning(f"⚠️ Error al renderizar ticket {ticket.ticket_number}: {str(e)[:80]}")
